...
from cosmopit import numMath,SplineFitting
...
def rhomo_d2(r,d2,threshold_d2=2.97):
    w=np.where(r >= 10)
    f=interpolate.interp1d(d2[w],r[w],bounds_error=False)
    return(f(threshold_d2))

def homogeneity_many_pairs(files,   
    cosmoFID={},cosmoGAU={},biasFID=None,sigpFID=None,zmid=0.5,kaiserFID=False,dampingFID=False,galaxyFID=False,biasGAU=2.0,sigpGAU=350.,kaiserGAU=False,dampingGAU=False,galaxyGAU=False,change_gauge=True,who_xi='ls',who_Nest='JC',threshold_d2=2.97):
    r,dd,rr,dr,ng,nr       = read_pairs(files[0])
...
def read_data(datafile,who_xi='ls',who_Nest='JC',combine=False,corrSS = False,rMin_fnct_nr_gal=5.0):
    # Read data
    if combine is False:
    ...
    nr = nr_gal(r,dd,rr,dr,who_xi=who_xi,who_Nest=who_Nest,correct_smallscales=corrSS,rMin=rMin_fnct_nr_gal)
...
def read_mocks(mockdir, cosmoFID={},cosmoGAU={},zmid=0.5,
    ...
    who_xi='ls',who_Nest='JC',who_give='d2',pairfiles='pairs_*.txt',combine=False,calc_xi=True,xi0Bool=True,change_gauge_xi=False,correct_smallscales=True,rMin_fnct_nr_gal=5.0,threshold_d2=2.97):
    ...
    if mockdir is None:
        ...
        if combine is False:
            ...
            r,mean_xi,sig_xi,mean_se,sig_se,mean_ss,sig_ss,covmat_xi,covmat_se,covmat_ss,cormat_xi,cormat_se,cormat_ss,all_rhxi,all_rhse,all_rhss,all_xi,all_se,all_ss=read_cute_many_se(mockfiles, 
                cosmoFID=cosmoFID,cosmoGAU=cosmoGAU,biasFID=biasFID,sigpFID=sigpFID,zmid=zmid,kaiserFID=kaiserFID,dampingFID=dampingFID,galaxyFID=galaxyFID,biasGAU=biasGAU,sigpGAU=sigpGAU,kaiserGAU=kaiserGAU,dampingGAU=dampingGAU,galaxyGAU=galaxyGAU,change_gauge=change_gauge,
                who_xi=who_xi,who_Nest=who_Nest,calc_xi=calc_xi,rMin_fnct_nr_gal=rMin_fnct_nr_gal)           
...
def NameZ2(minz=0.43,maxz=0.7,nbins=5):
    ...
    znames = []
    for i in np.arange(nbins): znames.append( 'z_'+str(np.round(zedge[i],3))+'_'+str(np.round(zedge[i+1],3)) )
...
def read_cute_many(files,
    ...
    who_Nest='JC',who_xi='ls',calc_xi=False,correct_smallscales=True,rMin_fnct_nr_gal=5.0,threshold_d2=2.97):
    ...
    for file_i in files:
        ...
        if calc_xi: all_nr[counter]  = nr_gal(r,dd,rr,dr,who_Nest=who_Nest,correct_smallscales=correct_smallscales,rMin=rMin_fnct_nr_gal) #nr_gal(r,dd,rr,dr,who_Nest=who_Nest,correct_smallscales=True) #N_of_xi(r,all_xi[counter])
        all_rhxi[counter]=rhomo_nr(r,all_xi[counter])
...
def read_cute_box(filename):
    blas   = np.loadtxt(filename)
    r      = blas[:,0]
    xi0    = blas[:,1]
    errxi0 = blas[:,2] 
    dd     = blas[:,3]

    return r,xi0,errxi0,dd

def read_cute_box_many(files):
    
    r,xi0,errxi0,dd = read_cute_box(files[0])
    nsim  = len(files)
    nbins = len(r)
    ...
    counter = 0
    for file_i in files:
        r,all_xi[counter],errxi0,dd  = read_cute_box(file_i)
        all_nr[counter]              = N_of_xi(r,all_xi[counter])
        all_rhxi[counter]=rhomo_nr(r,all_xi[counter])
...
def read_many_cute_box(dirfile,filenames='Bootstrap_0*.out',who_give='xi'):
    files = glob.glob(dirfile+filenames)
    r,mean_xi,sig_xi,mean_nr,sig_nr,mean_d2,sig_d2,covmat_xi,covmat_nr,covmat_d2,cormat_xi,cormat_nr,cormat_d2,all_rhxi,all_rhnr,all_rhd2,all_xi,all_nr,all_d2 = read_cute_box_many(files)
    nmock = len(all_xi)
    if who_give=='xi':
        covmatObs,meanObs,all_rhObs,all_Obs,rBoot,nmock = covmat_xi,mean_xi,all_rhxi,all_xi,r,nmock
    elif who_give=='nr':
        covmatObs,meanObs,all_rhObs,all_Obs,rBoot,nmock = covmat_nr,mean_nr,all_rhnr,all_nr,r,nmock
    elif who_give=='d2':
        covmatObs,meanObs,all_rhObs,all_Obs,rBoot,nmock = covmat_d2,mean_d2,all_rhd2,all_d2,r,nmock

    return covmatObs,meanObs,all_rhObs,all_Obs,rBoot,nmock
