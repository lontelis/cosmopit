...
#### MODELS and Bounds #####
#fid_params = {'Om':0.3, 'Ol':0.7, 'h':0.67 , 'ob':0.022,'ocdm':0.318 ,'ns':0.95, 'ln1e10As':3.094 , 'w':-1.}
...
def Sll_model_Aomol(datasets, variables = ['A','om','ol'], fidvalues = Sfid_params_Aomol):
    ...
    #A_mean,A_sigma2=1.4,0.02 # optimum! #2.15,2.25 still shit # 1.0,3.0
    #A      = pymc.Normal('A', A_mean,A_sigma2, value = Sfid_params_Aomol['A'], observed = 'A' not in variables)
    om     = pymc.Uniform('om', 0.0,1.0, value = Sfid_params_Aomol['om'], observed = 'om' not in variables)
    ol     = pymc.Uniform('ol', 0.0,1.0, value = Sfid_params_Aomol['ol'], observed = 'ol' not in variables)
    @pymc.stochastic(trace=True,observed=True,plot=False)
...
def ll_BR_nOmOX(datasets, variables = ['n', 'Om', 'OX'], fidvalues = fid_BR_nOmOX):
    if (isinstance(datasets, list) is False): datasets=[datasets]
    n_min,n_max   = 0.0, 0.2 #-1.0, 0.5 
    Om_min,Om_max = 0.0, 0.5 #+0.0, 1.0 
    OX_min,OX_max = 0.0, 0.2 #+0.0, 1.0 
...
def ll_test_linear(datasets, variables = ['a', 'b'], fidvalues = fid_test_linear):
    if (isinstance(datasets, list) is False): datasets=[datasets]
    a      = pymc.Uniform('a', 0.0,4.,   value = fid_test_linear['a'], observed = 'a' not in variables)
    b      = pymc.Uniform('b', 0.0,40.0, value = fid_test_linear['b'], observed = 'b' not in variables)
    @pymc.stochastic(trace=True,observed=True,plot=False)
    def loglikelihood(value=0, a=a,b=b):
        ll=0.
        pars = np.array([a,b])
         ...
         for j in np.arange(nplots-kk)+kk:
            ...
            if (i == j):
                ...
                if (var in chain.keys()) and (doit[j]==True):
                    ...
                    if paper2:
                      ylim([0.,1.1])
                      legend(loc=4,frameon=False,fontsize=12) #8 15
